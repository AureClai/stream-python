...
def initialize_simulation(Simulation):#, User):
    ...
    Simulation["Events"] = initial_Events(Simulation["Nodes"], Simulation["Links"])
    # ...
    # ---- (2) Initialisation of the arrivals at entries
    Simulation["Events"] = initial_arrivals(Simulation["Entries"], Simulation["Events"], Simulation["tmp"]["vehArray"])
    # ...
...
def initial_Events(Nodes, Links):
    ...
    for node in list(Nodes.keys()):
        ...
        if len(Nodes[node]["OutgoingLinksID"]) == 0:
            ...
            VehiclesNum = 1 + np.floor( np.max(np.array([Links[link]["Length"] for link in Nodes[node]["OutgoingLinksID"]]) * np.array([Links[link]["NumLanes"] for link in Nodes[node]["OutgoingLinksID"]]) * [Links[link]["FD"]["kx"] for link in Nodes[node]["OutgoingLinksID"]]))
        ...
        SupplyTimes = {}
        SupplyTimes["Downstream"] = -np.inf * np.ones([int(VehiclesNum), int(OutgoingLinksNum)])
            ...
            for managedLaneLink in Regulation['Args']['Links']:
                ...
                for time in Regulation['Args']['Times']:
                    ...
                    if activated:
                        ...
                        activated = True
                    Action['Args'] = {'LinkID' : managedLaneLink, 'Class' : Regulation['Args']['Class'], 'Display' : True}
                    Simulation['Actions'].append(Action)
    # ...
    # Sort actions
...
def recalculateAlphaOD(node, Links):
    ...
    if node["NumIncomingLinks"] >= 1: # this is a merge
        NumIncomingLanes = [Links[link]["NumLanes"] for link in node["IncomingLinksID"]]
